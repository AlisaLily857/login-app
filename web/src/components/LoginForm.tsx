...
const LoginForm: React.FC<LoginFormProps> = ({ 
  ...
  const strengthMap: Record<number, { label: string; color: string }> = {
      ...
      3: { label: t('password.strong'), color: '#3498db' },
      4: { label: t('password.veryStrong'), color: '#27ae60' },
    };
  ...
  const validateForm = (): boolean => {
    ...
    else if (password.length < 6) newErrors.password = t('validation.password');
    setErrors(newErrors);
  ...
  return (
    <div className="login-form-container">
      <form className="login-form" onSubmit={handleSubmit}>
        ...
        <button type="button" className="google-login-button" onClick={onGoogleLogin} disabled={isLoading}>
        ...
        <button type="button" className="github-login-button" onClick={onGitHubLogin} disabled={isLoading}>
        ...
        <button type="button" className="wechat-login-button" onClick={onWeChatLogin} disabled={isLoading}>
        ...
        <button type="button" className="phone-login-button" onClick={onPhoneLogin} disabled={isLoading}>
        ...
        <div className="form-group">
          ...
          <input id="email" type="email" className={`form-input ${errors.email ? 'error' : ''}`}
            placeholder={t('login.email')} value={email}
          ...
          {password && (
            <div className="password-strength">
              <div className="strength-bar-container">
                <div className="strength-bar" style={{ width: `${(passwordStrength.score / 4) * 100}%`, backgroundColor: passwordStrength.color }} />
              </div>
        ...
        <div className="form-options">
          <label className="remember-me"><input type="checkbox" /><span>{t('login.remember')}</span></label>
          <button type="button" className="forgot-password" onClick={onForgotPassword}>{t('login.forgot')}</button>
        </div>
        ...
        <div className="signup-link">{t('login.noAccount')}<button type="button" className="link-button" onClick={onRegister}>{t('login.register')}</button></div>
      </form>
    </div>
  );
};
